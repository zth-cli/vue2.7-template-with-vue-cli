...
import FromLayout from '../FormData/index'
export default {
  name: 'StepsFromData',
  data() {
    return {
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        ...
        this.postParams ? (this.formData = Object.assign({}, this.postParams, this.formData)) : ''
        console.log(this.formData)
        if (valid) {
          apiPost(this.postUrl, this.formData)
            .then((res) => {
              if (res.data.code === 1) {
