      ...
      <div class="curd_tools" ref="tools" :class="{ellipsis_curd_tools:expend}">
        <template v-for="(item, index) in searchDynamic">
          ...
          <template v-else-if="!item.dateSwitch&&item.type === 'daterange'">
            <div :key="'item' + index" class="curd_tool_item" ref="tool">
              <label class="label" v-if="mode !=='simple'">{{ item.label }}：</label>
              <el-date-picker v-model="fromData[item.name]" :type="item.type" :disabled="item.disabled" :value-format="item.format" clearable
                style="width: 260px" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" size="mini">
              </el-date-picker>
            </div>
          </template>
          <template v-else-if="item.type === 'select'">
            <div :key="'item' + index" class="curd_tool_item" ref="tool">
              <label class="label" v-if="mode !=='simple'">{{ item.label }}：</label>
              <el-select v-model="fromData[item.name]" :style="{width: width}"
                :placeholder="getPlaceholder(item)" collapse-tags filterable clearable
                :multiple="item.multiple ? true : false" size="mini">
          ...
          <template v-else-if="typeArr.includes(item.type)&&item.dateSwitch&&item.dateSwitch.length>1">
            <div :key="'item' + index" class="curd_tool_item" ref="tool">
             <el-button class="mini_btns" v-for="(el,i) in item.dateSwitch" size='mini' @click="switchIndex=i;switchDate(item.name,el, item.dateTypeParamName)"
              :type="switchIndex===i?'danger':'primary'" :key="i">
              {{el.type==='date'?'日':el.type==='month'?'月':el.type==='year'?'年': el.type==='dates'?'多日':'时间'}}
             </el-button>
             <el-date-picker :style="{width: width}" :type="item.dateSwitch[switchIndex].type" :disabled="item.disabled"
              :placeholder="getPlaceholder(item)" clearable v-model="fromData[item.name]" size="mini"
              :value-format="item.dateSwitch[switchIndex].type==='date'?'yyyy-MM-dd':item.dateSwitch[switchIndex].type==='month'?
              'yyyy-MM':item.dateSwitch[switchIndex].type==='year'?'yyyy':'yyyy-MM-dd'" ></el-date-picker>
            </div>
          </template>
          <template v-else>
            <div class="curd_tool_item" :key="index" ref="tool">
              <label class="label" v-if="mode !=='simple'">{{ item.label }}：</label>
              <el-input :style="{width: width}" v-model="fromData[item.name]" :placeholder="getPlaceholder(item)"
                size="mini"></el-input>
            </div>
          </template>
        </template>
...
export default {
  ...
  methods: {
    switchDate (prop, typeObj, typeParamName) {
      var format = typeObj.type === 'date' ? 'yyyy-MM-dd' : typeObj.type === 'month' ? 'yyyy-MM' : typeObj.type === 'year' ? 'yyyy' : 'yyyy-MM-dd'
    ...
    query () {
      ...
      for (const key in params) {
        if (params[key] instanceof Array) { // 对value为Array类型的进行字符串拼接
